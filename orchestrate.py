...
class InstanceManager:
    def __init__(self, compose_dir="compose_configs", build_images=False, logs_dir: str = "logs", start_delay_seconds: int = 0):
        self.compose_dir = Path(compose_dir)
        self.running_instances = {}
        self.compose_bin = detect_compose_bin()
        self.log_manager = LogManager(Path(logs_dir), self.compose_bin)
    ...
    def start_all(self):
        ...
        # Start instances sequentially with optional delay to avoid initial overload
        started_instances = {}
        for idx, cf in enumerate(compose_files):
            success, instance_name, compose_file = self.start_instance(cf)
            if success:
                self.running_instances[instance_name] = True
                started_instances[instance_name] = compose_file
            # sleep between launches except after the last one
            if idx < len(compose_files) - 1 and self.start_delay_seconds > 0:
                try:
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="logs",
        help="Base directory to store orchestrated service logs (default: logs)",
    )
    parser.add_argument(
        "--start-delay-seconds",
        type=int,
