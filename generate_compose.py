        ...
        "services": {
            f"prep_data_instance_{instance_id}": {
                ...
                "command": [
                    ...
                    'WORLD_DP_PATH="/data/$${WORLD_NAME}/datapacks"; '
                    'mkdir -p /data /data/plugins /data/skins "$${WORLD_DP_PATH}" && '
                    'chmod 777 /data /data/plugins /data/skins "/data/$${WORLD_NAME}" "$${WORLD_DP_PATH}" && '
                    ...
                    "fi; "
                    'if [ -d /source_datapacks ] && [ -n "$(ls -A /source_datapacks 2>/dev/null)" ]; then '
                    '  cp -r /source_datapacks/. "$${WORLD_DP_PATH}/"; '
                    "fi; "
                    'if [ -d /data/datapacks ] && [ -n "$(ls -A /data/datapacks 2>/dev/null)" ]; then '
                    '  cp -r /data/datapacks/. "$${WORLD_DP_PATH}/"; '
                    "  rm -rf /data/datapacks; "
                    "fi; "
                    'chmod -R 777 /data/plugins /data/skins "$${WORLD_DP_PATH}"',
                ],
                "volumes": [
                    f"{data_dir}:/data",
                    f"{project_root}/plugins:/source_plugins:ro",
                    f"{project_root}/skins:/source_skins:ro",
                    f"{project_root}/datapacks:/source_datapacks:ro",
                ],
                "restart": "no",
            },
