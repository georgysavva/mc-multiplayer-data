...
def analyze_video(args: Tuple[str, np.ndarray, np.ndarray, int, int]) -> VideoAnalysisResult:
    """
    ...
    if total_frames <= max_frames:
        ...
        frames_to_process = set(np.linspace(0, total_frames - 1, max_frames, dtype=int).tolist())
...
def process_split_folder(
    ...
    tasks = [(path, template_rgb, alpha_mask, max_frames, top_percentile) for path in all_video_paths]
    ...
    for (episode_id, instance_id), pair_files in episode_pairs.items():
        alpha_filename = os.path.basename(pair_files["Alpha"]) if pair_files["Alpha"] else None
        bravo_filename = os.path.basename(pair_files["Bravo"]) if pair_files["Bravo"] else None
        
        alpha_result = results_by_filename.get(alpha_filename) if alpha_filename else None
        bravo_result = results_by_filename.get(bravo_filename) if bravo_filename else None
...
def main():
    ...
    parser.add_argument(
        "--base-path",
        type=str,
        default="/mnt/data/dl3957/mc_multiplayer_v2_gpu",
    ...
    for split_folder in split_folders:
        ...
        save_results_json(output_path, water_episodes, non_water_episodes, args.threshold, args.top_percentile)
