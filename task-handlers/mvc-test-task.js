...
async function testMVCBehavior(bot, coordinator, otherBotName, durationMs) {
  console.log(`[${bot.username}] Starting ${durationMs/1000}s MVC behavior test with ${otherBotName}`);
  ...
  try {
    while (Date.now() - startTime < durationMs) {
      ...
      if (now - lastMovementChange > mvcTestConfig.random_movement_interval) {
        ...
        if (Math.random() < 0.8) { // 80% chance to move
          ...
          console.log(`[${bot.username}] MVC Test: Starting ${currentMovement} movement`);
        } else {
      ...
      if (now - lastMVCUpdate > mvcTestConfig.update_interval) {
        try {
          ...
          mvcStats.avg_fov_angle = ((mvcStats.avg_fov_angle * (mvcStats.total_ticks - 1)) + 
                                   mvcResult.mvcState.fov_angle_deg) / mvcStats.total_ticks;
          mvcStats.avg_distance = ((mvcStats.avg_distance * (mvcStats.total_ticks - 1)) + 
                                  mvcResult.mvcState.distance_to_partner) / mvcStats.total_ticks;
          ...
          if (mvcStats.total_ticks % 10 === 0) {
            console.log(`[${bot.username}] MVC Test Status:`, {
              ...
              current_distance: mvcResult.mvcState.distance_to_partner.toFixed(2),
    ...
    const fovViolationRate = (mvcStats.fov_violations / mvcStats.total_ticks) * 100;
    const distanceViolationRate = (mvcStats.distance_violations / mvcStats.total_ticks) * 100;
    const correctionRate = (mvcStats.corrections_applied / mvcStats.total_ticks) * 100;
