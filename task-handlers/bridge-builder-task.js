 ...
 */
async function buildCooperativeBridge(bot, coordinator, otherBotName, durationMs) {
  console.log(`[${bot.username}] Starting simple block placement episode with ${otherBotName}`);
  ...
  function yawToCardinal(yaw) {
    ...
    return new Vec3(dx || Math.sign(Math.cos(yaw)), 0, dz || Math.sign(Math.sin(yaw)));
  }
  ...
  try {
    ...
    while (Date.now() - startTime < durationMs) {
      ...
      if (!blockPlaced) {
        console.log(`[${bot.username}] Step 2: Attempting to place block in front using reliable method`);
        
        try {
          buildStats.placement_attempts++;
          
          // 1) Pick a block from inventory (first placeable item or fallback)
          let item = bot.inventory.items().find(i => bot.registry.items[i.type]?.stackSize); // any placeable
          ...
          if (!item && bot.game.gameMode === 'creative') {
            console.log(`[${bot.username}] Creative mode: Setting inventory slot with cobblestone`);
            try {
              await bot.creative.setInventorySlot(36, bot.registry.itemsByName['cobblestone'], 1);
              await sleep(100);
              item = bot.inventory.slots[36];
            } catch (creativeError) {
              console.log(`[${bot.username}] Creative inventory set failed:`, creativeError.message);
            }
          }
          
          if (!item) {
            console.log(`[${bot.username}] ❌ No placeable block found in inventory`);
            await sleep(2000);
            continue;
          }
          
          console.log(`[${bot.username}] Found placeable item: ${bot.registry.items[item.type].name} (${item.count} available)`);
          ...
          console.log(`[${bot.username}] Bot yaw: ${bot.entity.yaw.toFixed(2)} radians`);
          console.log(`[${bot.username}] Direction vector: (${dir.x}, ${dir.y}, ${dir.z})`);
          console.log(`[${bot.username}] Target front position: (${front.x}, ${front.y}, ${front.z})`);
          
          // 3) We'll place ON TOP of the ground in front (flat world ground is solid)
          const ground = bot.blockAt(front.offset(0, -1, 0));
          
          if (!ground) {
            console.log(`[${bot.username}] ❌ No ground block found at (${front.x}, ${front.y - 1}, ${front.z})`);
            await sleep(2000);
            continue;
          }
          
          console.log(`[${bot.username}] Ground block: ${ground.name} at (${ground.position.x}, ${ground.position.y}, ${ground.position.z})`);
          
          if (ground.boundingBox !== 'block') {
            console.log(`[${bot.username}] ❌ Ground is not a solid block (boundingBox: ${ground.boundingBox})`);
            await sleep(2000);
            continue;
          }
          ...
          if (existingBlock && existingBlock.name !== 'air') {
            console.log(`[${bot.username}] ❌ Target position already occupied by ${existingBlock.name}`);
          ...
          console.log(`[${bot.username}] Placing ${bot.registry.items[item.type].name} on top of ${ground.name}`);
          await bot.placeBlock(ground, new Vec3(0, 1, 0));
          
          buildStats.blocks_placed++;
          blockPlaced = true;
          console.log(`[${bot.username}] ✅ SUCCESS: ${bot.registry.items[item.type].name} placed at (${front.x}, ${front.y}, ${front.z})`);
          
        } catch (error) {
          console.error(`[${bot.username}] Block placement error:`, error.message);
          await sleep(2000);
        }
      }
      ...
      if (blockPlaced && otherBot && otherBot.entity) {
        console.log(`[${bot.username}] Step 3: Looking at ${otherBotName} to confirm placement`);
        try {
          await lookAtBot(bot, otherBotName, 120);
          await sleep(500);
          buildStats.eye_contact_duration += 500;
        } catch (error) {
          console.log(`[${bot.username}] Confirmation look failed:`, error.message);
        }
      }
    ...
    const eyeContactPercentage = (buildStats.eye_contact_duration / totalTime) * 100;
    
    console.log(`[${bot.username}] Simple block placement complete! Statistics:`, {
      blocks_placed: buildStats.blocks_placed,
      placement_attempts: buildStats.placement_attempts,
      success_rate: buildStats.placement_attempts > 0 ? ((buildStats.blocks_placed / buildStats.placement_attempts) * 100).toFixed(1) + '%' : '0%',
