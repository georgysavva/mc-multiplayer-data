...
EOF
  ...
  if ffmpeg -hide_banner -encoders 2>/dev/null | grep -q h264_nvenc; then
    ...
    echo "[client] Using NVENC hardware encoding (MKV) with wallclock timestamps - preset=$NVENC_PRESET tune=$NVENC_TUNE"
    ffmpeg -hide_banner -loglevel info -y \
      -use_wallclock_as_timestamps 1 \
      -f x11grab -video_size "${WIDTH}x${HEIGHT}" -framerate "$FPS" -i "${DISPLAY}.0" \
      -copyts -vsync 0 \
      -c:v h264_nvenc -preset "$NVENC_PRESET" $TUNE_OPTS -bf 0 -pix_fmt yuv420p "$RECORDING_PATH" &
  else
