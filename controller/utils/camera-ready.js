...
async function useRcon(host, port, password, task) {
  ...
  try {
    ...
    try {
      ...
      console.warn('[camera-ready] Failed to close RCON connection:', err?.message || err);
    }
  }
}
 ...
 */
async function waitForCameras(rconHost, rconPort, rconPassword, maxRetries, checkInterval) {
  const cameraNames = ['CameraAlpha', 'CameraBravo'];
  
  console.log(`[camera-ready] Waiting for cameras: ${cameraNames.join(', ')}`);
  console.log(`[camera-ready] Max retries: ${maxRetries}, check interval: ${checkInterval}ms`);
  
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      const list = await useRcon(rconHost, rconPort, rconPassword, (rcon) => rcon.send('list'));
      ...
      if (foundCameras.length === cameraNames.length) {
        console.log(`[camera-ready] All cameras present: ${foundCameras.join(', ')}`);
        return true;
      }
      ...
      console.warn(`[camera-ready] Failed to query player list (attempt ${attempt}/${maxRetries}):`, err?.message || err);
    }
  ...
  console.error(`[camera-ready] Timeout: Cameras did not join within ${maxRetries} attempts`);
  return false;
}
