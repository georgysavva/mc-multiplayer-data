...
async function digBlock(bot, blockPos) {
  try {
    ...
    console.log(`[${bot.username}] ❌ Error digging block at ${blockPos}: ${error.message}`);
    return false;
  }
}
...
async function digDownOneBlock(bot) {
  ...
  console.log(`[${bot.username}] 📍 Current position: ${currentPos.x.toFixed(2)}, ${currentPos.y.toFixed(2)}, ${currentPos.z.toFixed(2)}`);
  ...
  if (success) {
    ...
    console.log(`[${bot.username}] 📍 New position: ${newPos.x.toFixed(2)}, ${newPos.y.toFixed(2)}, ${newPos.z.toFixed(2)}`);
    console.log(`[${bot.username}] 📏 Y-level change: ${(newPos.y - currentPos.y).toFixed(2)}`);
  }
...
async function mineTunnelTowards(bot, targetPos, maxBlocks = 20) {
  ...
  while (distanceTraveled < horizontalDistance - 1.5 && blocksMined < maxBlocks) {
    const myPos = bot.entity.position.clone();
    
    console.log(`[${bot.username}] 📍 Current position: ${myPos.x.toFixed(2)}, ${myPos.y.toFixed(2)}, ${myPos.z.toFixed(2)}`);
    console.log(`[${bot.username}] 📏 Distance to target: ${myPos.distanceTo(targetPos).toFixed(2)} blocks`);
    ...
    console.log(`[${bot.username}] ⛏️ Digging 2x1 tunnel at X=${nextBlockX}, Z=${nextBlockZ}`);
    ...
    for (const blockPos of blocksToDigPositions) {
      ...
      if (block && block.name !== 'air' && block.name !== 'cave_air') {
        console.log(`[${bot.username}] 🔨 Digging ${block.name} at ${blockPos}`);
        ...
        try {
          ...
          await bot.dig(block);
          blocksMined++;
          console.log(`[${bot.username}] ✅ Mined block ${blocksMined}: ${block.name}`);
          await sleep(100);
        } catch (digError) {
          console.log(`[${bot.username}] ⚠️ Failed to dig block at ${blockPos}: ${digError.message}`);
        }
      } else {
    ...
    console.log(`[${bot.username}] 📊 Progress: ${distanceTraveled.toFixed(2)}/${horizontalDistance.toFixed(2)} blocks`);
  ...
  console.log(`[${bot.username}]    Distance traveled: ${finalDistance.toFixed(2)} blocks`);
...
function getOnMinePhaseFn(
  ...
  return async function onMinePhase(otherBotPosition) {
    ...
    console.log(`[${bot.username}] 🎬 MINING EPISODE - Episode ${episodeNum}, Iteration ${iterationID}`);
    ...
    try {
      ...
      console.log(`[${bot.username}] ⚠️ Could not equip tool: ${toolError.message}`);
    }
    ...
    console.log(`[${bot.username}] 👀 STEP 5: Looking towards other bot's direction...`);
    try {
      const otherEntity = bot.players[otherBotName]?.entity;
      if (otherEntity) {
        actualOtherBotPosition = otherEntity.position.clone();
        ...
        await bot.look(targetYaw, 0, true);
        console.log(`[${bot.username}] ✅ Looking towards other bot at yaw ${targetYaw.toFixed(2)}`);
        await sleep(1000); // Hold the look for 1 second
      }
    } catch (lookError) {
      console.log(`[${bot.username}] ⚠️ Could not look towards other bot: ${lookError.message}`);
    }
    ...
    console.log(`[${bot.username}] 🚇 STEP 7: Mining 2x1 tunnel towards midpoint...`);
    const miningResult = await mineTunnelTowards(bot, midpoint, 20);
    
    console.log(`[${bot.username}] ✅ Mining complete! Mined ${miningResult.blocksMined} blocks`);
    ...
    console.log(`[${bot.username}] 📊 Final stats: ${miningResult.blocksMined} blocks mined, ${miningResult.distanceMined.toFixed(2)} blocks traveled`);
